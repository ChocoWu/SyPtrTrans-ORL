...
dynet_config.set(autobatch=1, mem='5120', random_seed=joint_config['random_seed'], requested_gpus=1, gpu_mask=[1])
...
from orl_eval import OrlEval
from io_utils import to_set, get_logger
from shift_reduce import ShiftReduce
import time

logger = get_logger('transition', log_dir='log',
                    log_name='trains-{}.log'.format(time.strftime("%Y-%m-%d-%H-%M-%S", time.localtime())))
...
class SynPtrTrans(object):

    def __init__(self, n_words, action_dict, role_type_vocab, dep_type_vocab, pos_dict, pretrained_vec=None,
                 udog_type_vocab=None):
        ...
        if joint_config['use_char_rnn']:
            self.char_embed = nn.Embedding(joint_config['n_chars'],
                                           ...
                                           trainable=True)
            self.char_rnn = nn.MultiLayerLSTM(joint_config['char_embed_dim'], joint_config['char_rnn_dim'],
                                              bidirectional=True)
        ...
        if joint_config['random_word_embed']:
            ...
            self.word_linear = nn.Linear(joint_config['word_embed_dim'] * 2, joint_config['word_embed_dim'],
                                         activation='relu')
        ...
        self.encoder_output_dim = joint_config['encoder_project'] if joint_config[
                                                                         'encoder_project'] > 0 else self.encoder_output_dim

        # shift reduce parser
        self.shift_reduce = ShiftReduce(joint_config, self.encoder_output_dim, action_dict, role_type_vocab,
                                        dep_type_vocab, udog_type_vocab, pos_type_vocab=pos_dict)
    ...
    def __call__(self, toks, chars, act_ids, acts, sent_range, pos_list, dep_list, dep_head,
                 sent_len, dss=None, relations=None,
                 neibour_index=None, neibour_type=None):

        context_emb = self.input_embed(toks, chars, pos_list, dep_list, dep_head,
                                       sent_range, is_train=True, return_sent_vec=False)

        log_prob_list, loss_roles, loss_position, pred_dss, pred_holders, pred_targets, pred_pairs, pred_acts = \
            self.shift_reduce(toks, context_emb, oracle_actions=act_ids, oracle_action_strs=acts,
                              is_train=True, relations=relations, sent_len=sent_len, gold_dss=dss,
                              neibour_index=neibour_index, neibour_type=neibour_type, pos_list=pos_list)
        act_loss = -dy.esum(log_prob_list)
        role_loss = dy.esum(loss_roles) if loss_roles else 0
        position_loss = -dy.esum(loss_position) if loss_position else 0
        loss = act_loss + 0.8 * role_loss + 0.8 * position_loss + self.regularization_loss(0.2)

        return loss

    def decode(self, toks, chars, act_ids, acts, sent_range, pos_list, dep_list, dep_head, mtype='dev',
               sent_len=None, dss=None, relations=None,
               neibour_index=None, neibour_type=None):
        dy.renew_cg()

        context_emb = self.input_embed(toks, chars, pos_list, dep_list, dep_head, sent_range, is_train=False,
                                       return_sent_vec=False, mtype=mtype)

        log_prob_list, loss_roles, loss_position, pred_dss, pred_holders, pred_targets, pred_pairs, pred_acts = \
            self.shift_reduce(toks, context_emb, oracle_actions=act_ids, oracle_action_strs=acts,
                              is_train=False, relations=relations, sent_len=sent_len, gold_dss=dss,
                              neibour_index=neibour_index, neibour_type=neibour_type, pos_list=pos_list)

        return pred_dss, pred_holders, pred_targets, pred_pairs
