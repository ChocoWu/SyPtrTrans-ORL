...
class ShiftReduce(object):
    ...
    def __init__(self, config, encoder_output_dim, action_dict, opi_role_vocab):
        ...
        hidden_input_dim = bi_rnn_dim * 1 + lmda_dim * 3 \
        ...
        self.pn3_linear = nn.Linear(lmda_dim + bi_rnn_dim,
                                    1, activation='elu')

    def pointer_network(self, current_state, input_hidden_states, max_sent_len, idx):
        x = [self.pn3_linear(ops.cat([t, current_state], dim=0)) for t in input_hidden_states]
    ...
    def __call__(self, toks, hidden_state_list, oracle_actions=None,
        ...
        while True:
            ...
            state_embed = ops.cat([beta_embed, lmda_embed, sigma_e_embed,
                                   alpha_e_embed, action_embed, out_embed], dim=0)
            if is_train:
            ...
            if self.act.is_no_start(action):
                ...
                if not buffer.is_empty():
                    ...
                    scores = self.pointer_network(ds_start_hx, hidden_state_list, sent_len, ds_start_idx)
                    if is_train:
                ...
                if not buffer.is_empty():
                    ...
                    scores = self.pointer_network(ht_start_hx, hidden_state_list, sent_len, ht_start_idx)
                    if is_train:
