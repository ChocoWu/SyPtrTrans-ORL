...
class Linear(object):
    def __init__(self, n_in, n_out, bias=True, activation='linear', model=None, init_w=None, init_type='glorot'):
        if model is None:
        ...
        if init_w is not None:
            ...
            self.W = model.add_parameters((n_out, n_in), init=init_type, name='linearW')
        self.bias = bias
        self.act = activation
        if bias:
...
class Embedding(object):
    ...
    def __call__(self, input):
        if isinstance(input, int):
            output = dy.lookup(self.embed, input, update=self.trainable)
        elif isinstance(input, list):
            output = [dy.lookup(self.embed, x, update=self.trainable) for x in input]
        elif isinstance(input[0], list):
            output = [[dy.lookup(self.embed, x, update=self.trainable) for x in a] for a in input]
        else:
...
class StackLSTM(object):
    ...
    def iter(self):
        for (hx, cx), idx, tx in zip(self.states, self.indices, self.types):
            yield hx, idx, tx

    def push(self, input, idx, tx):
        '''
    ...
    def last_state(self):
        return self.states[-1][0], self.indices[-1], self.types[-1]
...
class Buffer(object):
    ...
    def update(self, hidden_state_list):
        self.hidden_states = hidden_state_list
...
class LambdaVar(object):
    ...
    def __init__(self, bi_rnn_dim):
        ...
        self.lambda_type = LambdaVar.OPINION
    ...
    def pop(self):
        ...
        self.lambda_type = LambdaVar.OPINION
        return var, idx, self.lambda_type
    ...
    def is_ht(self):
        return self.lambda_type == LambdaVar.HT

    # def is_target(self):
    #     return self.lambda_type == LambdaVar.TARGET
...
class RCGA(object):
    def __init__(self, in_features, out_features, dropout, concat=True, model=None):
        if model is None:
            model = pm.global_collection()
        ...
        self.linear_1 = Linear(out_features*2+50, 1, init_type='uniform')
        self.linear_2 = Linear(out_features+50, out_features, init_type='uniform')

    def __call__(self, h, udog_graph_embed=None, neibour_index=None, neibour_type=None,
                 sent_len=None):
        h_prime = []
        for i in range(sent_len):
            temp = []
            Wh = []
            for m, n in zip(neibour_index[i], neibour_type[i]):
                Wh.append(ops.cat([h[m], udog_graph_embed(n)], dim=0))
                t = self.linear_1(ops.cat([h[i], h[m], udog_graph_embed(n)], dim=0))
                t = relu(t)
                temp.append(t.value())
            softmax_att = dy.softmax(dy.inputTensor(temp), 0)
            new_h_d = dy.esum([self.linear_2(m)*n for m, n in zip(Wh, softmax_att)])
            h_prime.append(new_h_d)
        h_prime.extend(h[sent_len:])
        return h_prime
...
class MultiRCGA(object):
    def __init__(self, n_layer, in_features, out_features, dropout, concat=True, model=None):
        if model is None:
            model = pm.global_collection()
        self.n_layer = n_layer
        self.gat_builders = [RCGA(in_features, out_features, dropout, concat=concat, model=model)]
        for _ in range(n_layer - 1):
            self.gat_builders.append(RCGA(in_features, out_features, dropout, concat=concat, model=model))

    def __call__(self, h, udog_graph_embed=None, neibour_index=None, neibour_type=None,
                 sent_len=None):

        for gat in self.gat_builders:
            h = gat(h, udog_graph_embed=udog_graph_embed, neibour_index=neibour_index, neibour_type=neibour_type,
                    sent_len=sent_len)
        return h


class Graph(object):
    def __init__(self, config, model=None, neibour_index=None, neibour_type=None, hidden_state_list=None,
                 sent_len=None, udog_type_vocab=None, update_func=None):
        if model is None:
            self.model = pm.global_collection()
        else:
            self.model = model

        self.neibour_index = neibour_index
        self.neibour_type = neibour_type

        self.sent_len = sent_len
        self.udog_graph_embed = Embedding(len(udog_type_vocab), config['udog_embed_dim'], trainable=True)
        self.udog_type_vocab = udog_type_vocab
        self.hidden_states = hidden_state_list

        self.update_func = update_func

    def update_opi(self, opn_start, opn_end, role_start, role_end, role_type):
        for i in range(opn_start, opn_end+1):
            self.neibour_index[i].append(opn_end)
            self.neibour_type[i].append(self.udog_type_vocab.get_index('OPINION'))
            self.neibour_index[opn_end].append(i)
            self.neibour_type[opn_end].append(self.udog_type_vocab.get_index('OPINION'))
        for i in range(role_start, role_end + 1):
            self.neibour_index[i].append(role_end)
            self.neibour_type[i].append(self.udog_type_vocab.get_index('ROLE'))
            self.neibour_index[role_end].append(i)
            self.neibour_type[role_end].append(self.udog_type_vocab.get_index('ROLE'))
        self.neibour_index[opn_end].append(role_end)
        self.neibour_type[opn_end].append(self.udog_type_vocab.get_index(role_type))
        self.neibour_index[role_end].append(opn_end)
        self.neibour_type[role_end].append(self.udog_type_vocab.get_index(role_type))

    def update_hidden_state(self):
        history_hidden_state = self.hidden_states
        self.hidden_states = self.update_func(history_hidden_state,
                                              self.udog_graph_embed, self.neibour_index, self.neibour_type,
                                              self.sent_len)

    def graph_pool(self):
        return dy.emax(self.hidden_states)
